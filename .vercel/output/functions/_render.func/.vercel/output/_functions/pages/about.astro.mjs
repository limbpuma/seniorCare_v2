...
import { $ as $$AboutSection, a as $$ImageCarrousel2, b as $$BannerSlider } from '../chunks/ImageCarrousel_2_DvW63y5J.mjs';
import { $ as $$Icon, a as $$Button, t as texts, b as $$Layout } from '../chunks/Layout_D2qM0WZL.mjs';
import { $ as $$DiscoverIcons } from '../chunks/DiscoverIcons_C1d_1cwU.mjs';
import { $ as $$ParallaxVideo, a as $$Testimonial } from '../chunks/ParallaxVideo_dkUhj0zF.mjs';
...
const $$ExpertSection = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${renderComponent($$result, "Section", $$Section, { "background": "white", "padding": "large", "width": "full", "height": "auto", "id": "experience-section", "animation": "slide-in" }, { "default": ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="container mx-auto px-4 max-w-screen-xl"> <div class="flex flex-col lg:flex-row items-center lg:items-start gap-8"> <div class="w-full lg:w-1/2"> ${renderComponent($$result2, "TextBlock", $$TextBlock, { "subtitle": subtitle, "title": title, "description": description, "alignment": "left", "textColor": "text-deep-blue" })} <ul class="space-y-4 mb-8 lg:ml-8"> ${listItems.map((item) => renderTemplate`<li class="flex items-center text-ag-body-text font-ag-body-text mb-6 sm:mb-8 md:mb-10 max-w-3xl mx-auto"> ${renderComponent($$result2, "Icon", $$Icon, { "icons": [{ name: "check" }], "size": "medium", "variant": "accent", "customSize": "text-xl" })} <span class="ml-2">${item}</span> </li>`)} </ul> <div class="flex flex-wrap items-center lg:ml-8"> ${renderComponent($$result2, "Button", $$Button, { "href": buttonLink, "variant": "primary", "size": "medium" }, { "default": ($$result3) => renderTemplate`${buttonText}` })} </div> </div> <div class="w-full lg:w-1/2 relative"> <div class="relative aspect-w-16 aspect-h-9"> <img${addAttribute(mainImage, "src")} alt="Main Image" class="w-full h-full object-cover rounded-lg shadow-lg" loading="lazy"> <img${addAttribute(overlayImage, "src")} alt="Overlay Image" class="absolute bottom-0 right-0 w-2/3 h-2/3 object-cover rounded-lg shadow-lg border-4 border-white transform translate-y-1/4 translate-x-1/6" loading="lazy"> </div> </div> </div> </div> ` })}`;
